...
from app.schemas.auth import LoginData, Token, RefreshTokenRequest, ForgotPasswordRequest, ResetPasswordRequest, \
    LogoutRequest
...
async def register(*, session: Session = Depends(get_session), user_in: UserCreate):
    user = User(**user_in.model_dump(exclude_unset=True), password_hash=get_password_hash(user_in.password), email_verified=False)
    session.add(user)
    session.commit()
...
def login(*, session: Session = Depends(get_session), data: LoginData):
    ...
    db_rt = RefreshToken(user_id=user.id, token=refresh_token, expires_at=datetime.utcnow() + timedelta(days=7))
    session.add(db_rt)
    session.commit()
...
def refresh_tokens(*, session: Session = Depends(get_session), req: RefreshTokenRequest):
    payload = decode_token(req.refresh_token)  # Asegúrate que esta función exista
    ...
    nr = RefreshToken(user_id=user_id, token=new_refresh, expires_at=datetime.utcnow() + timedelta(days=7))
    session.add(nr)
    session.commit()
...
def verify_email(*, session: Session = Depends(get_session), token: str = Query(...)):
...
async def forgot_password(*, session: Session = Depends(get_session), req: ForgotPasswordRequest):
...
def reset_password(*, session: Session = Depends(get_session), req: ResetPasswordRequest):
...
def logout(*, session: Session = Depends(get_session), req: LogoutRequest):
    rt = session.exec(select(RefreshToken).where(RefreshToken.token == req.refresh_token)).one_or_none()
